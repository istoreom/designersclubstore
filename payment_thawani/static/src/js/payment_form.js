odoo.define('payment_thawani.payment_form', function (require) {
"use strict";
...
const thawaniMixin = {
    ...
    payEvent: function (ev) {
        ...
        // first we check that the user has selected a thawani as s2s payment method
        // Default odoo send data with post method we need to send form data with the GET method
        if (checked_radio.length === 1 && checked_radio.data('provider') === "thawani") {
            var form = this.el;
            var self = this;
            if (ev.type === 'submit') {
                    ...
                    if ($tx_url.length === 1) {
                        ...
                        return this._rpc({
                            ...
                            if (result) {
                                ...
                                newForm.setAttribute("method", "get"); // set it to post
                                ...
                                var action_key = $(newForm).find('input[name="thawani_publishable_key"]').val();
                                ...
                                $(newForm).find('input').remove(); // remove all the input that should be removed
                                newForm.innerHTML = "<input type='hidden' name='key' value='" + action_key + "'/>"
                                if(action_url) {
...
checkoutForm.include(thawaniMixin);
manageForm.include(thawaniMixin);
});
